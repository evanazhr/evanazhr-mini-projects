...
export default function MonoalphabeticCipher() {
  ...
  const accentColor = pageHeaderData?.accent || 'var(--chart-5)'
          ...
          <div className="flex flex-col gap-8">
              ...
              <CardContent className="p-5 flex flex-col gap-6">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-end gap-3">
                  <div className="flex-1 flex flex-col gap-1.5">
                    <Label htmlFor="keyword-input" className="text-xs font-bold uppercase tracking-wider text-foreground/70">
                  ...
                  <Button
                    ...
                    variant="neutral"
                    className="sm:w-auto shrink-0 flex items-center gap-2 font-bold"
                  >
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="overflow-x-auto border-2 border-border rounded-base shadow-shadow">
                    <table className="w-full border-collapse text-center text-xs font-black min-w-[700px]">
                      <thead>
                        <tr className="bg-main border-b-2 border-border">
                          <th className="p-2.5 border-r-2 border-border bg-foreground text-background shrink-0 sticky left-0 z-10 w-24 select-none">
                            Sebelum
                          </th>
                          {ALPHABET.split("").map((char, i) => (
                            <td key={i} className="p-2 border-r-2 border-border text-main-foreground bg-main/10 font-bold select-none">
                              {char}
                            </td>
                          ))}
                        </tr>
                        ...
                        <tr className="bg-secondary-background">
                          ...
                          {cipherKey.split("").map((char, i) => {
                            ...
                            return (
                              <td 
                                key={i} 
                                className={`p-2 border-r-2 border-border font-black transition-colors ${
                                  isChanged ? 'bg-main text-main-foreground' : 'text-foreground/60 bg-secondary-background'
                                }`}
                              >
                                {char}
                              </td>
            ...
            <Card className="p-0 overflow-hidden border-2 border-border shadow-shadow bg-secondary-background">
              ...
              <CardContent className="p-5 flex flex-col gap-6">
                ...
                {/* Mode Selector Tabs */}
                <div className="flex border-2 border-border rounded-base shadow-shadow overflow-hidden">
                  <button
                    type="button"
                    onClick={() => setMode("encrypt")}
                    className={`flex-1 py-3 text-sm font-black uppercase tracking-wider transition-all select-none border-r-2 border-border cursor-pointer text-center ${
                      mode === "encrypt" 
                  ...
                  <button
                    ...
                    className={`flex-1 py-3 text-sm font-black uppercase tracking-wider transition-all select-none cursor-pointer text-center ${
                      mode === "decrypt" 
                        ? 'bg-foreground text-background' 
                        : 'bg-secondary-background text-foreground hover:bg-main/10'
                    }`}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {/* Left: Input */}
                  <div className="flex flex-col gap-2">
                    <Label htmlFor="input-text" className="text-xs font-bold uppercase tracking-wider text-foreground/70">
                    ...
                    <textarea
                      ...
                      className="w-full p-3 font-medium text-sm text-foreground bg-secondary-background border-2 border-border rounded-base outline-none focus-visible:bg-secondary-background focus-visible:shadow-[4px_4px_0px_var(--page-accent)] transition-all resize-none"
                    />
                  </div>
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <textarea
                      ...
                      className="w-full p-3 font-black text-sm text-foreground bg-main/5 border-2 border-border rounded-base outline-none resize-none select-all"
                    />
                  </div>
                </div>
              </CardContent>
            ...
            {/* Info Box */}
            <div className="flex flex-col gap-2 p-4 border-2 border-border bg-secondary-background rounded-base shadow-shadow">
              ...
              <ul className="text-xs font-semibold text-foreground/70 list-disc pl-4 space-y-2 leading-relaxed">
                <li>
                  <strong className="text-foreground">Kata Kunci</strong>: Huruf unik dari kata kunci diletakkan di awal alfabet baru, lalu huruf alfabet lainnya yang belum dipakai akan diisikan berurutan. Misalnya, kunci <strong className="text-foreground">“RAHASIA”</strong> memetakan huruf abjad pertama ke <strong className="text-foreground">R, A, H, S, I</strong> diikuti oleh abjad tersisa (<strong className="text-foreground">B, C, D, E, F, G, J, K, L, M, N, O, P, Q, T, U, V, W, X, Y, Z</strong>).
                </li>
                <li>
                  <strong className="text-foreground">Acak Kunci (Shuffle)</strong>: Mengacak 26 huruf alfabet secara acak tanpa kata kunci spesifik untuk mendapatkan keamanan pemetaan yang lebih tinggi.
                </li>
                <li>
